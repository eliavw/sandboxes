...
```bash
git remote add origin git@github.com:eliavw/sandboxes.git
git remote -v
...
```bash
git remote add origin git@github.com:eliavw/sandboxes.git; git remote -v; git push origin master
```
...
```bash
conda env create -f dependencies-deploy.yaml -n sandboxes
```

To additionally add the packages which are relevant for the development phase, do:

```bash
conda activate sandboxes
conda env update -n sandboxes -f dependencies-develop.yaml
```
...
```bash
conda activate sandboxes
python -m ipykernel install --user --name sandboxes --display-name "sandboxes"
```
...
1. Go to the [Travis](https://travis-ci.com/eliavw/sandboxes) page of this repo.
2. See if it ran.
...
and your site should be online at; [https://eliavw.github.io/sandboxes/](https://eliavw.github.io/sandboxes/). 
...
> A repo where I can play around with stuff, cf. https://eliavw.github.io/sandboxes
